 ...
 *    Array of URL paths to be ignored (e.g. paths for static content).
 * @return {Function(req, res, next)} Express middleware.
 */
module.exports = function(logger, blacklist) {
   ...
   */
  function isUrlBlackedList(url) {
    return _blacklist.some(url.startsWith.bind(url));
  }

  return function loggingMiddleware(req, res, next) {
    if (!isUrlBlackedList(req.originalUrl)) {
